...
public final class RenderUtils {
	...
	public static void renderHearts(final Minecraft mc, final float partialTick, @Nullable final Mob mob, final long remainingMs) {
		...
		tess.setColor4f(brightness, brightness, brightness, alpha);
		TextureRegistry.guiSpriteAtlas.bind();

		final int health = mob.getHealth();
		final int maxHealth = CONTAINER_STYLE.value == 2 ? health : mob.getMaxHealth();

		final int containerHealth = CONTAINER_STYLE.value == 2 ? Math.max(health, 1) : maxHealth;
		final int containerMaxHealth = Math.max(maxHealth, 1);
		drawHearts(tess, containerHealth, containerMaxHealth, heartsFlash ? CONTAINER_BLINKING : CONTAINER);
		drawHearts(tess, health, maxHealth, getMobHeartTex(mob));
	...
	private static void drawHearts(final TessellatorShader tess, final int health, final int containers, final IconCoordinate[] icons) {
		...
		for (int i = 0; i < rows; ++i) {
			...
			final int rowIdx = containerRows * i;
			...
			for (int j = 0; j < heartsToDraw; ++j) {
				...
				final double zOffset = (rowIdx + j) * 0.001 * (fillOrderDown ? -renderOrderZ : renderOrderZ);
	...
	private static float getBrightness(@NonNull final Entity entity, final float partialTick) {
		...
		return applyGamma(HEALTH_FULL_BRIGHT.value ? HEALTH_BRIGHTNESS.value : entity.getBrightness(partialTick), gamma);
	}
